...
export function DocumentTool({ state, input, output, elapsedTime }: DocumentToolProps) {
  const documentTitle = output && typeof output === 'object' && !Array.isArray(output) && 'title' in output
    ? output.title
    : input?.title;
  const documentUrl = output && typeof output === 'object' && !Array.isArray(output) && 'sourceUrl' in output
    ? output.sourceUrl
    : null;
          ...
          <div className="not-prose max-w-prose">
            <div className="flex items-center gap-2 text-muted-foreground text-sm">
              ...
              <span className="flex-1">
                Reading document... {elapsedTime > 0 && `${elapsedTime}s`}
              </span>
            </div>
          </div>
        </motion.div>
